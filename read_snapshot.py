...
def read_whole_grid(data_dir="data/", snapshot=MOST_RECENT_SNAPSHOT, include_ghost=0,
    ...
    # Read grid info
    # TODO: time variable needs to be handled differently if we have multiple snapshots
    grid_info_path = data_dir + "grid_info.ac"
    NX, NY, NZ, GRID_SIZE, COMP_DOMAIN_SIZE_X, COMP_DOMAIN_SIZE_Y, COMP_DOMAIN_SIZE_Z, PAD_SIZE, BOUND_SIZE, DX, DY, DZ, time = read_grid_info(
        grid_info_path)
    ...
    # Read grid data
    if include_lnrho:
        lnrho = read_grid_dat(lnrho_path, NX, NY, NZ, PAD_SIZE, BOUND_SIZE, COMP_DOMAIN_SIZE_X,
                              COMP_DOMAIN_SIZE_Y, COMP_DOMAIN_SIZE_Z, include_ghost, ldouble=ldouble)
    else:
        lnrho = np.zeros((xx.size, yy.size, zz.size))

    if include_ux:
        uu_x = read_grid_dat(uu_x_path, NX, NY, NZ, PAD_SIZE, BOUND_SIZE, COMP_DOMAIN_SIZE_X,
                             COMP_DOMAIN_SIZE_Y, COMP_DOMAIN_SIZE_Z, include_ghost, ldouble=ldouble)
    else:
        uu_x = np.zeros((xx.size, yy.size, zz.size))

    if include_uy:
        uu_y = read_grid_dat(uu_y_path, NX, NY, NZ, PAD_SIZE, BOUND_SIZE, COMP_DOMAIN_SIZE_X,
                             COMP_DOMAIN_SIZE_Y, COMP_DOMAIN_SIZE_Z, include_ghost, ldouble=ldouble)
    else:
        uu_y = np.zeros((xx.size, yy.size, zz.size))

    if include_uz:
        uu_z = read_grid_dat(uu_z_path, NX, NY, NZ, PAD_SIZE, BOUND_SIZE, COMP_DOMAIN_SIZE_X,
                             COMP_DOMAIN_SIZE_Y, COMP_DOMAIN_SIZE_Z, include_ghost, ldouble=ldouble)
    else:
        uu_z = np.zeros((xx.size, yy.size, zz.size))

    uu_tot = np.sqrt(np.power(uu_x, 2.0) +
                     np.power(uu_y, 2.0) + np.power(uu_z, 2.0))
...
def check_inf_nan(lnrho, uu_x, uu_y, uu_z, uu_tot, snapshot=MOST_RECENT_SNAPSHOT):
    ...
    print "rho_min = %e ; rho_max = %e ; rho_rms = %e" % (
        rho_min, rho_max, rho_rms)
    print "lnrho_min = %e ; lnrho_max = %e ; lnrho_rms = %e" % (
        lnrho_min, lnrho_max, lnrho_rms)
    print "uu_tot_min = %e ; uu_tot_max = %e ; uu_tot_rms = %e" % (
        uu_tot_min, uu_tot_max, uu_tot_rms)
    print "uu_x_min = %e ; uu_x_max = %e ; uu_x_rms = %e" % (
        uu_x_min, uu_x_max, uu_x_rms)
    print "uu_y_min = %e ; uu_y_max = %e ; uu_y_rms = %e" % (
        uu_y_min, uu_y_max, uu_y_rms)
    print "uu_z_min = %e ; uu_z_max = %e ; uu_z_rms = %e" % (
        uu_z_min, uu_z_max, uu_z_rms)
    ...
    print "lnrho_nan = %i ; lnrho_inf = %i ; rho_nan = %i ; rho_inf = %i ; uu_tot_nan = %i ; uu_tot_inf = %i" % (
        lnrho_nan, lnrho_inf, rho_nan, rho_inf, uu_tot_nan, uu_tot_inf)
    print "uu_x_nan = %i ; uu_x_inf = %i ; uu_y_nan = %i ; uu_y_inf = %i ; uu_z_nan = %i ; uu_z_inf = %i" % (
        uu_x_nan, uu_x_inf, uu_y_nan, uu_y_inf, uu_z_nan, uu_z_inf)
