...
vel_tot = np.sqrt(np.power(velx, 2.0) +
                  np.power(vely, 2.0) + np.power(velz, 2.0))
vel_tot0 = np.sqrt(np.power(velx0, 2.0) +
                   np.power(vely0, 2.0) + np.power(velz0, 2.0))
...
data = dict(Density=rho, Velocity_x=velx, Velocity_y=vely,
            Velocity_z=velz, Velocity=vel_tot)
bbox = np.array(
    [[-Lbox/2.0, Lbox/2.0], [-Lbox/2.0, Lbox/2.0], [-Lbox/2.0, Lbox/2.0]])
