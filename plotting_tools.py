...
def plot_simple(var0, var, slice_plane, slice_pos, curve_axis, title, xx, yy, zz):
    ...
    if curve_axis == "x":
        plt.figure()
        plt.plot(xx, var0[:, slice_pos, slice_pos],
                 xx, var[:, slice_pos, slice_pos])
        plt.xlabel('x')
        plt.ylabel(title)
        plt.title(title)
#		plt.figure()
#		plt.plot(xx, diff_var[:,slice_pos,slice_pos])
        ...
        plt.figure()
        plt.plot(yy, var0[slice_pos, :, slice_pos],
                 yy, var[slice_pos, :, slice_pos])
        plt.xlabel('y')
        plt.ylabel(title)
        plt.title(title)
    elif curve_axis == "z":
        plt.figure()
        plt.plot(zz, var0[slice_pos, slice_pos, :],
                 zz, var[slice_pos, slice_pos, :])
        plt.xlabel('z')
        plt.ylabel(title)
        plt.title(title)
    elif curve_axis == "xyz":
