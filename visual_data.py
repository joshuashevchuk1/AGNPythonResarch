...
xx0, yy0, zz0, lnrho0, uu_x0, uu_y0, uu_z0, uu_tot0 = rvar.read_whole_grid(
    snapshot=0, include_ghost=include_ghost)
xx, yy, zz, lnrho, uu_x, uu_y, uu_z, uu_tot = rvar.read_whole_grid(
...
if view == "uu_x":
    acplt.plot_simple(uu_x0, uu_x, slice_plane, slice_pos,
                      curve_axis, view, xx, yy, zz)

if view == "uu_y":
    #	draw_isosurface(diff_uu_y, 10)
    acplt.plot_simple(uu_y0, uu_y, slice_plane, slice_pos,
                      curve_axis, view, xx, yy, zz)

if view == "uu_z":
    #	draw_isosurface(diff_uu_z, 10)
    acplt.plot_simple(uu_z0, uu_z, slice_plane, slice_pos,
                      curve_axis, view, xx, yy, zz)

if view == "lnrho":
    #draw_isosurface(lnrho, 10)
    acplt.plot_simple(lnrho0, lnrho, slice_plane, slice_pos,
                      curve_axis, view, xx, yy, zz)

if view == "exp_lnrho":
    #	draw_isosurface(np.exp(lnrho), 10)
    acplt.plot_simple(np.exp(lnrho0), np.exp(lnrho), slice_plane,
                      slice_pos, curve_axis, view, xx, yy, zz)
...
if view == "uu_tot":
    ...
    acplt.plot_simple(uu_tot0, uu_tot, slice_plane,
                      slice_pos, curve_axis, view, xx, yy, zz)

if view == "uu_x_lnrho":
    acplt.plot_simple(uu_x, lnrho, slice_plane, slice_pos,
                      curve_axis, view, xx, yy, zz)

if view == "uu_y_lnrho":
    acplt.plot_simple(uu_y, lnrho, slice_plane, slice_pos,
                      curve_axis, view, xx, yy, zz)

if view == "uu_z_lnrho":
    acplt.plot_simple(uu_z, lnrho, slice_plane, slice_pos,
                      curve_axis, view, xx, yy, zz)

if view == "helicity":
    oox, ooy, ooz = der.curl(uu_x, uu_y, uu_z, dx, dy, dz)
    helical_grid = der.dot(uu_x, uu_y, uu_z, oox, ooy, ooz)
    oo_tot = np.sqrt(np.power(oox, 2.0) +
                     np.power(ooy, 2.0) + np.power(ooz, 2.0))
...
if view == "powersp_all":
    ...
    print "uu_x..."
    vkx_x, vky_x, vkz_x, kx_x, ky_x, kz_x = pows.powerspectrum(
        uu_x, COMP_DOMAIN_SIZE_X, COMP_DOMAIN_SIZE_Y, COMP_DOMAIN_SIZE_Z, Lbox)
    print "uu_y..."
    vkx_y, vky_y, vkz_y, kx_y, ky_y, kz_y = pows.powerspectrum(
        uu_y, COMP_DOMAIN_SIZE_X, COMP_DOMAIN_SIZE_Y, COMP_DOMAIN_SIZE_Z, Lbox)
    print "uu_z..."
    vkx_z, vky_z, vkz_z, kx_z, ky_z, kz_z = pows.powerspectrum(
        uu_z, COMP_DOMAIN_SIZE_X, COMP_DOMAIN_SIZE_Y, COMP_DOMAIN_SIZE_Z, Lbox)
